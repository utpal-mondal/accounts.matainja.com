...
class Site extends CI_Model
{
	...
	public function getstaff($id = NULL) {
        ...
        $q = $this->db->get_where('staff_info', array('user_id' => $id), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
    ...
    public function getReference($field) {
		if($field=='inv'){
			...
			$this->db->limit(1);
			$last_invoice_ref = $this->db->get('sma_invoice');
			$last_ref_no = $last_invoice_ref->result_array();
			$inv_ref_no = explode('/', $last_ref_no[0]['reference_no']);
			$ref_no = 'MT/';
            $ref_no .= date('Y') . "/" . date('m') . "/". date('d');
			if(isset($inv_ref_no[3]) && $inv_ref_no[3] == date('d') && !isset($inv_ref_no[4]) && $inv_ref_no[4]==''){
				$ref_no .= "/". 1;
			}else if(isset($inv_ref_no[4]) && $inv_ref_no[4]!=''){
				$ref_no .= $inv_ref_no[4]+1;
			}
			if ($this->getInvoiceByReference($ref_no)) {
            	$ref_no .= mt_rand(0, 9);
        	}
            return $ref_no;
		}else{
	...
	public function getInvoiceByReference($ref) {
        $this->db->like('reference_no', $ref, 'before');
        $q = $this->db->get('sma_invoice', 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
