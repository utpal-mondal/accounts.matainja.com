...

{
        ...
        $default_key=$this->config->item('api_key');
          ...
          if($key == $default_key)
            ...
            { 
              ...
              $email=($this->input->get_post('user_email')!='')?$this->input->get_post('user_email'):'';

              

               $app_password=($this->input->get_post('app_pswd')!='')?$this->input->get_post('app_pswd'):'';

               $appnew_pswd=md5($app_password);

       

          

                $result=$this->App_model->check_user($email,$appnew_pswd);
  ...
  public function testdailystaffattendancereport()
 ...
 //   $attendance_result=$this->App_model->testmatch_attendancedate();

    /*print_r($attendance_result);

    die();*/

$todays_date=date('d.m.Y');//date('d.m.Y',strtotime("-1 days"));

 //todays_date=date('d.m.Y',strtotime("-1 days"));
      ...
      foreach($attendance_result as $data)

       {
        ...
        if($data->active==1){
         ...
         $attendance_newarray['total_present_today'] = $present;

         $attendance_newarray['total_absent_today'] = $abs;
  ...
  /*  else{
	  ...
	  ////test dailystaffattendancereport end
...

  {
		...
		$todays_date=date('d.m.Y',strtotime("-2 days"));
		$a=$b=$c=0;
		$abs=0;
		...
		{*/
		//print_r($attendance_result['attendances']);
		if(!empty($attendance_result['attendances']))

   {
      foreach($attendance_result['attendances'] as $data)

       {
        if($data->active==1){

          if(strtotime($data->date_entry)==strtotime('2018-05-11'))//$todays_date
          ...
          if($data->is_late==1)

            {
                ...
                $attendance_newarray['late'][$a]=$attendance_array;
				$a++;
                   ...
                   $attendance_newarray['present'][$b]=$attendance_array;
                   $b++;
                  }
...

           {
            ...
            $attendance_array['u_name']=$data->first_name.' '.$data->last_name;
                   ...
                   $attendance_newarray['absent'][$c]=$attendance_array;
  $c++;
           }
         ...
         $attendance_newarray['total_absent_today'] = (int)$attendance_result['total_user'] - (int)$present;
...
//print_r($attendance_newarray); die('sebr');
