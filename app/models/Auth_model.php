...
class Auth_model extends CI_Model {
    ...
    // Additional properties used in constructor
...
public function getUsers_group($id = FALSE)

    {
        ...
        $query = $this->db->select($this->tables['groups'] . '.id as id, ' . $this->tables['groups'] . '.name, ' . $this->tables['groups'] . '.description')

            //->where($this->tables['groups'] . '.id', $id)
               ->from('sma_users')
               ->join('sma_groups', 'sma_groups.id = sma_users.group_id')
               ->where('sma_users.id', $id)
               ->get();
