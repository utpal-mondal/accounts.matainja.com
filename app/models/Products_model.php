...
class Products_model extends CI_Model
{
    ...
    public function getCategoryProducts($category_id)
    {
	 ...
	 public function getImeiNoByID($id)
    {
        $query = $this->db->get_where('imei', array('product_id' => $id));
		//print_r($query); 
        if ($query->num_rows() > 0) {
            return $query->result(); 
        }
	...
	public function updateimeistatus($id){
		
	$this->db->update('imei',array('status' =>'sold'), array('id' => $id));
	}
	...
	public function addiemi($data){
		$this->db->insert('imei', $data); 
