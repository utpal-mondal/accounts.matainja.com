	 ...
	 function SendEmailStaff($user_info,$get_attendance,$post_response)
	      ...
	      if(!empty($attendec_record))
	      {
	      	$in_out_time =0;
                    ...
                    $parse_data = array(
                      ...
                      $attnd_time =date("d M Y",strtotime($attendec_record['entry_time']));
	 	

					  $msg=file_get_contents('./themes/' . $this->theme . 'email_templates/attendance_notification.html');
					  $admin_msg_body=file_get_contents('./themes/' . $this->theme . 'email_templates/attendance_notification-with-hour.html');
					  $messagenew = $this->parser->parse_string($msg, $parse_data);
					   $admin_message_parse = $this->parser->parse_string($admin_msg_body, $parse_data);

 
 
		                $subject = "[ Attendance Notification  ] ".$attnd_time." ".$prefix_text." " .$user_info->first_name . ' ' . $user_info->last_name;
		               ...
		               if(isset($user_info->project_manager) && $user_info->project_manager>0)
		               {
                         ...
                         $subject = "[ Team Attendance Notification ] ".$attnd_time." ".$prefix_text." " .$user_info->first_name . ' ' . $user_info->last_name;
                          $this->sma->send_email($pminfo->email,$subject,$messagenew);
		               ...
		               $this->sma->send_email("matainja.staff@gmail.com",$subject,$admin_message_parse);
		    }
      ...
      function set_attendance(){
		  ...
		  $post_response=array();
			...
			$is_late=($this->input->get_post('is_late')!='')?$this->input->get_post('is_late'):0;
			$profile_img=($this->input->get_post('profile_img')!='')?$this->input->get_post('profile_img'):"";
			$note=($this->input->get_post('note')!='')?$this->input->get_post('note'):"";
			...
			$post_response['profile_img'] = $profile_img;
			$post_response['is_late'] = $is_late;
	     		...
	     		if(!empty($user_info) && $user_id>0)
					...
					$data = array(
	     				...
	     				if($insert_attendance)
						{
							...
							$result['status']=1;
	     					...
	     					$data_img = array(
								'attendence_id'=>$insert_attendance,
								'user_img'=>$profile_img,
								
						);

	     					$img_path_save ="/home3/newmata/accounts.matainja.com/attendence-img/".$insert_attendance."-profile.txt";
	     					$Imgfile = fopen($img_path_save, "w") or die("Unable to open file!");
                            fwrite($Imgfile, $profile_img);
                            fclose($Imgfile);
                            //$insert_attendance_img=$this->Staff_model->AddImgAttendence($data_img);
					...
					if(isset($rfid) && $rfid!="")
					{
						...
						$data = array(
							...
							$insert_attendance=$this->Staff_model->NewrfId($data);	
							$result['status']=2;
