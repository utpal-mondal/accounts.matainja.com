...
class Auth extends MY_Controller
{
    ...
    function __construct()
    {
        ...
        // Set allow_reg from settings
        $this->allow_reg = $this->Settings->allow_reg;
    }
    ...
    function getUsers()
    {
        if ( ! $this->Owner) {
		 ...
		 $edit_link = anchor('auth/profile/$1', '<i class="fa fa-edit"></i>' . lang('edit_user'), 'class="sledit"');
    ...
    function login($m = NULL)
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            log_message('debug', 'LOGIN ATTEMPT - Identity: ' . $identity . ', Password length: ' . strlen($password));
            log_message('debug', 'LOGIN ATTEMPT - Remember: ' . ($remember ? 'true' : 'false'));
            log_message('debug', 'LOGIN ATTEMPT - Ion Auth config - Identity field: ' . $this->config->item('identity'));
            log_message('debug', 'LOGIN ATTEMPT - Ion Auth config - Hash method: ' . $this->config->item('hash_method'));
            
            // Check if user exists before attempting login
            $user_check = $this->ion_auth->user($identity)->row();
            if ($user_check) {
                log_message('debug', 'LOGIN ATTEMPT - User found: ID ' . $user_check->id . ', Username: ' . $user_check->username . ', Email: ' . $user_check->email);
                log_message('debug', 'LOGIN ATTEMPT - User active: ' . $user_check->active);
                log_message('debug', 'LOGIN ATTEMPT - User group_id: ' . $user_check->group_id);
                log_message('debug', 'LOGIN ATTEMPT - Stored password hash: ' . $user_check->password);
                log_message('debug', 'LOGIN ATTEMPT - Hash length: ' . strlen($user_check->password));
            } else {
                log_message('debug', 'LOGIN ATTEMPT - User NOT found with identity: ' . $identity);
            }

            if ($this->ion_auth->login($identity, $password, $remember)) {
                log_message('debug', 'LOGIN SUCCESS - User logged in successfully');
                ...
                // Log login failure details
                $ion_auth_errors = $this->ion_auth->errors();
                log_message('debug', 'LOGIN FAILURE - Ion Auth errors: ' . $ion_auth_errors);
                
                // Test password verification manually for debugging
                if ($user_check) {
                    $test_password = "12345678";
                    $salt = "sma_2023";
                    $sha1_hash = sha1($test_password . $salt);
                    $verify_sha1 = ($sha1_hash === $user_check->password);
                    log_message('debug', 'LOGIN FAILURE - Manual SHA1 verification: ' . ($verify_sha1 ? 'SUCCESS' : 'FAILED'));
                    log_message('debug', 'LOGIN FAILURE - Expected SHA1 hash: ' . $sha1_hash);
                    log_message('debug', 'LOGIN FAILURE - Actual stored hash: ' . $user_check->password);
                }
                
                $this->session->set_flashdata('error', $ion_auth_errors);
                redirect('login');
            }
        } else {
            // Log form validation failure
            log_message('debug', 'LOGIN FAILURE - Form validation failed');
            log_message('debug', 'LOGIN FAILURE - Validation errors: ' . validation_errors());
            log_message('debug', 'LOGIN FAILURE - Session error: ' . $this->session->flashdata('error'));
