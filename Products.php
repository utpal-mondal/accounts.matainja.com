...

{
...

    {
       ...
       // $single_barcode = anchor('products/print_barcodes/$1', '<i class="fa fa-print"></i> ' . lang('print_barcode_label'));
		...
		<ul class="dropdown-menu pull-right" role="menu">
			...
			<li><a href="' . site_url('products/imei/$1') . '"><i class="fa fa-plus-circle"></i> ' . lang('add_imei') . '</a></li>';
        ...
        if ($warehouse_id) {
            ...
            ->select($this->db->dbprefix('products') . ".id as productid,{$this->db->dbprefix('products')}.code as code, {$this->db->dbprefix('products')}.name as name, {$this->db->dbprefix('brands')}.name as brand, {$this->db->dbprefix('categories')}.name as cname, cost as cost, price as price, COALESCE(wp.quantity, 0) as quantity, {$this->db->dbprefix('units')}.code as unit", FALSE)
    ...
    function getImeiDetails($id=NULL){
		echo $id;die();
		
		 $this->load->library('datatables');
         $this->datatables
            ->select("id,product_id,imei_no,status") 
            ->from("imei")
			->where('product_id', $id);
    ...
    function imei($id=NULL){
	
		 $data=array();
		 $this->sma->checkPermissions();
		 
		 $imei = $this->input->post('imei_no');
		if(!empty($imei)){
			
		foreach($imei as $imeino){
			if($imeino !='')
		     {
			...
			$query=$this->products_model->addiemi( $data);
			if($query){
				
				$this->session->set_flashdata('msg', 'IMEI no added successfully');
				}
			 }
			}
			
			redirect('products/imei/'.$id, 'refresh');		
		}

		 $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('products'), 'page' => lang('add_imei')), array('link' => '#', 'page' => lang('add_imei')));

         $meta = array('page_title' => lang('add_imei'), 'bc' => $bc);
		 $this->data['id'] = $id;
		 $this->page_construct('products/emi', $meta, $this->data);
    ...
    /* ---------------------------------------------------------------- */	
     ...
     function updatestatus($id=NULL){
		 $query=$this->products_model->updateimeistatus($id);
...

    {
        ...
        $this->sma->checkPermissions('index');
          ...
          $imei_details = $this->products_model->getImeiNoByID($id); 
		    
		  $this->data['imei'] = $imei_details;
